  ...
  useEffect(() => {
    const getProfile = async () => {
      try {
        ...
        if (user) {
          // Preluăm rolul din metadate pentru a ști în ce tabel să căutăm
          const role = user.user_metadata?.role || 'pacient';
          const tableName = (role === 'doctor' || role === 'medic') ? 'doctori' : 'pacienti';
    ...
    <SakuraLayout>
      <div style={styles.container}>
        <div style={styles.backWrapper}>
          <button style={styles.circleBackButton} onClick={() => navigate(-1)} title="Înapoi">
            ←
          </button>
        </div>
        ...
        <div style={styles.card}>
          ...
          <p style={{ color: '#8a7d84' }}>
            Rol: <strong style={{color: '#ff6b81', textTransform: 'capitalize'}}>{profile?.role}</strong>
          </p>
...
const styles: { [key: string]: React.CSSProperties } = {
  container: { padding: '60px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '80vh' },
  backWrapper: { width: '100%', maxWidth: '500px', textAlign: 'left', marginBottom: '15px' },
  circleBackButton: {
    width: '40px', height: '40px', borderRadius: '50%', backgroundColor: 'white',
    border: '1px solid #f0f0f0', color: '#4d444a', fontSize: '18px', cursor: 'pointer',
  ...
  avatarLarge: { 
    ...
    backgroundColor: '#ffb7c5', color: 'white', fontSize: '40px', 
    fontWeight: 'bold', display: 'flex', alignItems: 'center', 
    justifyContent: 'center', margin: '0 auto 20px',
    boxShadow: '0 8px 20px rgba(255, 183, 197, 0.4)'
  },
  divider: { height: '1px', backgroundColor: '#f9f9f9', margin: '25px 0' },
  labelStyle: { fontSize: '11px', color: '#b2a4ac', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: '700' },
  dataStyle: { fontSize: '16px', color: '#4d444a', fontWeight: '600', marginTop: '5px' },
  infoGrid: { display: 'grid', gridTemplateColumns: '1fr', gap: '20px', textAlign: 'center' },
  editBtn: {
    padding: '12px 30px',
